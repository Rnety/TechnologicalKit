...
![image-20211019151720820](C:\Users\333\AppData\Roaming\Typora\typora-user-images\image-20211019151720820.png)
...
要是我们想要得到对应的URL我们怎么办，我们应该使用this.$route，其实我们的router对象里面有对应的route这个route对应我们的使用的活跃的路由
...
export default {
  ...
  computed:{
    userId(){
      return this.$route.params.userId;  //一一对应哦
    }
  }
}
</script>
...
![image-20211019181006571](C:\Users\333\AppData\Roaming\Typora\typora-user-images\image-20211019181006571.png)

- **app代表我们的逻辑代码**
- **manifest是我们相互引用的底层支撑**
- **vendor是我们的第三方工具**
...
![image-20211019182316782](C:\Users\333\AppData\Roaming\Typora\typora-user-images\image-20211019182316782.png)
...
<template>
  ...
  <router-link to="/home/message">首页</router-link>
...
<template>
  <div id="app">
    <router-link to="/home">首页</router-link>
    <router-link to="/about">小玩意</router-link>
    <button @click="userClick">啊？</button>
    <button @click="profileClick">哦？</button>
    <router-view></router-view>
  </div>
</template>
...
Promise是**ES6异步编程的一种解决方案**。那什么时候我们会来处理异步事件呢?
...
正常情况下，不会有什么问题，可以正常运行并且获取我们想要的结果。但是，这样额代码难看而且不容易维护。
...
这是我们过去的处理方式，我们将它换成Promise代码这个例子会让我们感觉脱裤放屁，多此一举
...
当我们的一般有异步操作的时候对即将进行的异步操作进行封装

- new的时候我们开始保存我们的一些状态信息
- 并执行我们传入函数，在执行传入函数的时候，我们会传入两个参数，一个resolve和reject，resolve和reject（可传可不传）本身也是函数
- 看作网络请求会传回一些data，我们假如在data下面有还没处理的时候，假如我们这里我们不想我们的data在这里处理，我们的在resolve(data)，然后我们的数据就会把我们的data把这个请求的的数据传入到下面的的then里面，我们在then里面处理我们的
- **将网络请求和数据处理进行分离**
...
Vuex提出使用**单—**状态树,什么是单—状态树呢?

英文名称是Single Source of Truth，也可以翻译成单—数据源。但是，它是什么呢?我们来看一个生活中的例子。
...
假如我们的单一状态树里面的某些数据需要经过处理才被用

> 这不是改变我们state的值

则我们在getter里面的可以构建方法然后在直接和state一样使用

**当我们的方法不能确定返回值的时候，我们只能返回一个函数，在返回的函数里面定义我们的操作**
...
<script>
import Helloworld from "./components/vuex/Helloworld";
...
export default {
  ...
  methods:{
    subtraction(count){
      this.$store.commit('decrement',count)  //方法一
      this.$store.commit({  //方法二
          type:'decrement',
          count
      })  
    }
...
```js
  decrement(state,count){  //自动传入state
      this.state.counter -= count  //方法一
      this.state.counter -= payload.count //方法二,方法二的对象传递我们喜欢改成payload,方法二又和添加对象的方法有点相似
    }
```
...
这个时候,我们通常会以对象的形式传递,也就是payload是分一个对象.这个时候可以再从对象中取出相关的信息.
...
export default {
  ...
  methods:{
    subtraction(){
      const count={key:value,key:value} //假如是一个数组类型
...
```js
  decrement(state,count){  //自动传入state
      this.state.students.push(count) //假如我们的Student是一个数组
    }
```

```js
  decrement(state,count){  //自动传入state
      this.state.stu = count.stu //假如我们的stu是一个对象的一个key
    }
```
...
```js
updateInfo(参数(可写可不写)){
    this.$store.dispatch('actions里面对应的方法名'，参数(可写可不写)).then(msg=>{
        console.log(msg)
    })
}
```

> 记得参数上下对应，且参数也可以写成对象型

**后端**

```js
action里面对应的方法名(context,参数(可写可不写)){
    return new Promise((resolve,reject)=>{
        异步操作代码
        resolve("成功信息")  //对应promise中用resolve把数据丢到then里面处理，这里进行了中转
    })
}
```
...
Module是模块的意思,为什么在Vuex中我们要使用模块呢Vue使用单—状态树,那么也意味着很多状态都会交给Vuex来管理.
...
最后我们的a,b都会被放到state里面

```js
{{$store.state.a.name}}
```

而对应的mutations是先大后小的顺序进行查找，不能重名
...
const moduleA ={
 getters:{
   ...
   return getters.fullname+'222' //复用
}
 fullname3(state,getters,rootState){
   return getters.fullname+rootState.counter //拿到根属性下的state
}
    }
 actions: {
     incrementIfOddOnRootSum({state,commit,rootState})  //异步操作context.commit时候不是root了，而是只能调用自己模块的mutations
     if(...)
        .....
 }
}
...
我们除了State卸载index.js里面，其他的全部放到外面导出最后在index.js 导入引用
...
**选择三**:官方在Vue1.x的时候推出了Vue-resource.>Vue-resource的体积相对于jQuery小很多.

另外Vue-resource是官方推出的.为什么不选择它呢?

- 在Vue2.0退出后, Vue作者就在GitHub的Issues中说明了去掉vue-resource,并且以后也不会再更新.

- 那么意味着以后vue-resource不再支持新的版本时,也不会再继续更新和维护.对以后的项目开发和维护都存在很大的隐患.
...
- `img`又有对应的划分，比如我们的`tabbar`是一类组件，就应该划分对应的文件夹
